...
class Config_Editor:
    ...
    def config_menu(self, section, choices, callback=None, callback_args=None):
        ...
        prompt_choices.insert(
            0, {"name": "back" if section != "main" else "done", "value": "back"})
        ...
        if choice == "back":
            if section != "main":
                ...
                color_print([("LimeGreen", Localizer.get_localized_text(
                    "prints", "config_modification", "config_saved"))])
                return
        else:
            if isinstance(choices[choice], dict):
                self.config_menu(choice, choices[choice], callback=self.config_menu, callback_args=(
                    section, choices, callback, callback_args))
            else:
